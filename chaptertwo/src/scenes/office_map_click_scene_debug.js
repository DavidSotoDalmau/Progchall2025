...
this.npcActiveIds = new Set(); // ids en uso
  ...
  this.resumeFrom = data?.resumeFrom || null;}
...
spawnNPC(fromNodeId = null) {
  ...
  if (freeId == null) return null; // no hay ids libres
...
despawnNPC(npc) {
  ...
  if (npc?.id != null) this.npcActiveIds.delete(npc.id); // ← libera el id
}
 checkEncounters() {
  for (const npc of this.npcs) {
    ...
    if (d <= this.encounterRadius) {
      ...
      this.despawnNPC(npc);
      this.scene.start('DuelScene', { npcId: passedId, resumeFrom }); // ← PASA coords
      break;
    }
  }
}
